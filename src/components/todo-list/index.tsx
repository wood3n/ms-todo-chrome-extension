...
/** 任务列表 */
function TodoList({ onClose }: Props) {
  const [currentTodoData, changeCurrentTodoData] = useTodoList(state => [state.currentTodoData, state.changeCurrentTodo]);
  ...
  const defaultTodos = todos.filter(item => item.wellknownListName === "defaultList");
    ...
    <div className="flex h-full flex-col items-start">
      ...
      <ScrollShadow hideScrollBar className="w-full overflow-auto">
        <Listbox
          ...
          items={todos}
          selectedKeys={currentTodoData.id ? new Set([currentTodoData.id]) : undefined}
